...
from apps.common import choices as common_choices


class SoftDeletionQuerySet(QuerySet):
    def delete(self):
        return super(SoftDeletionQuerySet, self).update(
            object_status=common_choices.ObjectStatusTypeChoices.DELETED
        )
...
class SoftDeletionManager(models.Manager):
    def get_queryset(self):
        return SoftDeletionQuerySet(self.model).filter(
            object_status=common_choices.ObjectStatusTypeChoices.ACTIVE
        )
    def complete(self):
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            self.id = datetime.now().strftime("%d%m%Y%H%M%S%f") + get_random_string(
                length=5, allowed_chars="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
            )
        super(TimeStampModel, self).save(*args, **kwargs)
    
    def delete(self, hard_delete=False):
        if not hard_delete:
            self.object_status = common_choices.ObjectStatusTypeChoices.DELETED
            self.save()
        else:
            super().delete()
